...
class HoHLocalAnalyzer:
    ...
    def load_metadata(self):
        ...
        try:
            ...
            print(f"📘 Loaded metadata.json from: {meta_path}")
        except Exception as e:
            print("⚠️ Unable to load metadata.json:", e)
            self.meta = {}
    ...
    def translate_building(self, building_key):
        ...
        return f"{category} ({size}), Level {level} – {era}"
    ...
    def create_summary_dashboard(self):
        ...
        for c in self.cities:
            ...
            for category, sizes in groups.items():
                ...
                ghtml += f"<h4>{category} — Total: {total}</h4>\n<ul>"

                for size, info in sizes.items():
                    level_list = ", ".join(str(l) for l in info["levels"])
                    ghtml += f"<li>{size} — {info['count']} (levels: {level_list})</li>"
